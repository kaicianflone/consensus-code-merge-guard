...
const TOP = new Set(['board_id','change_summary','constraints','persona_set_id']);
...
function validate(i){ if(!i||typeof i!=='object') return 'input must be object'; let e=rejectUnknown(i,TOP,'input'); if(e) return e; if(typeof i.board_id!=='string') return 'board_id required'; if(!i.change_summary||typeof i.change_summary!=='object') return 'change_summary required'; e=rejectUnknown(i.change_summary,CHG,'change_summary'); if(e) return e; return null; }
...
export async function handler(input, opts={}) {
  ...
  try {
    ...
    const votes=makeVotes(ps,input.change_summary,input.constraints||{});
    ...
    const { personas, updates }=updateReputations(ps.personas,votes, ag.final_decision);
